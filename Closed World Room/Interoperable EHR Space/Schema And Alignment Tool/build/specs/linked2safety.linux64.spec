...
a = Analysis([os.path.join(appdir,'linked2safety.py')],
             pathex=[os.path.join(builddir,'linux64')],
             hiddenimports=[],
             hookspath=None)
...
exe = EXE(pyz,
          a.scripts,
          exclude_binaries=1,
          name=os.path.join('build', 'pyi.linux2', 'linked2safety.linux64', 'linked2safety.linux64.bin'),
          debug=False,
          strip=None,
          upx=True,
          console=True , 
...
coll = COLLECT(exe,
               ...
               strip=None,
               upx=True,
               name=os.path.join(builddir, 'linux64','linked2safety'))
