...
a = Analysis([os.path.join(appdir,'linked2safety.py')],
             pathex=[os.path.join(builddir,'win64')],
             hiddenimports=[],
             hookspath=None)
...
a.datas += extra_datas('.', os.path.join(curdir, 'libs', 'Default'))
a.datas += extra_datas('.', os.path.join(curdir, 'libs', 'Win64'))
...
exe = EXE(pyz,
          a.scripts,
          exclude_binaries=1,
          name=os.path.join('build', 'pyi.win32', 'linked2safety.win64', 'linked2safety.win64.exe'),
          debug=False,
          strip=None,
          upx=True,
          console=False , 
...
coll = COLLECT(exe,
               ...
               strip=None,
               upx=True,
               name=os.path.join(builddir, 'win64','linked2safety.win64'))
