#    AdverseEventsProfileBarchart.py - creates a barchart to display the number of adverse events of each profile of a user
#
...
def main():
	...
	AdverseEventsDict={} #key=profile_name, value=number of adverse events of profile
	#count adverse events in adverse event history field of profiles
	...
	AdverseEventsDict = store_AEs_of_each_profile(user_id,records,records2,cursor,AdverseEventsDict)
	AdverseEventsDict = count_adverse_events(AdverseEventsDict)
	conn.close()	
...
def count_adverse_events(AdverseEventsDict):
#input: dictionary (key=profile name, value=list of AEs of a profile)
#output: dictionary (key=profile name, value=number of AEs of a profile)
	keys = AdverseEventsDict.keys()
	for key in keys:
		AdverseEventsDict[key] = len(AdverseEventsDict[key])
 	return AdverseEventsDict
...
def BarChart(AdverseEventsDict,output_file):
		...
		ax.set_title('Number of Adverse Events of Each Profile',fontstyle='normal')
