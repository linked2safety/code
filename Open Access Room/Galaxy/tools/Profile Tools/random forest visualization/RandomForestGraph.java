/* RandomForestGraph.java - visualize a random forest using a graph 
...
public class RandomForestGraph {   
	...
	static HashMap<String,String> edges; //An edge is identified using the pair of nodes of the edge
					     //key: the pair of nodes e.g. "node0 -> node1"
	...
	public final static void main(String[]args) throws Exception
	 {	    
	    ...
	    results_file = args[0]; //output txt file of a Random Forest tool 
	    ...
	    out.write("digraph G {\n");
	    create_random_forest_graph(in,out);	 
	    out.write("\n}");
	    out.close();
	    in.close();  
	 }

 static void create_random_forest_graph(BufferedReader in,FileWriter out) throws IOException
  {
	//create graph of first decision tree
	//create graphs of remaining decision trees
	...
	p1=Pattern.compile("^decision tree.*$");
	...
	create_1st_decision_tree_graph(in,out);
	node_id++; 
	cluster_id++;	
	root_node_added=false; 	
	create_decision_trees_graphs(in,out);
  }

 static void create_decision_trees_graphs(BufferedReader in, FileWriter out) throws IOException
  {	
	String line;
	do	
	{
	 ...
	 line = create_decision_tree_graph(in,out);
    	
	 cluster_id++;	//increment cluster id
	 node_id++; //increment node id
	 root_node_added = false;
	}
	while(line!=null);
  }

 static void create_1st_decision_tree_graph(BufferedReader in, FileWriter out) throws IOException
 {
	 /* get next line
	    while (line != "decision tree")
	    { 
	      if(line is first condition of a rule)
	      then add the condition to rule linkedlist
		   get nodes and edges from the rule linkedlist
	      else if(line is middle condition of a rule)
		   then add the condition to rule linkedlist
	      else if(line is a rule with one condition only)
		   then add the rule to rule linkedlist
			get nodes and edges from the rule linkedlist
	      else if(line contains the last condition and consequent of rule)
		   then add the last condition and consequent to rule linkedlist
	      else if(line contains a middle condition of a rule)
		   then add the middle condition to rule linkedlist
	      else //line is empty, nothing is done
		   {}
	    }	
	    write nodes and edges to graphviz file
	 */
	 LinkedList<String> rule; 
	 String line;
	...
	while(!line.equals("decision tree"))
    { 
		  try
		  {
 ...
 static String create_decision_tree_graph(BufferedReader in, FileWriter out) throws IOException
 {
	 /* get next line from input file
	    while (line != "decision tree" && line != null)
	    { 
	      if(line is first condition of a rule)
	      then 
	          add the condition to rule linkedlist
		      get nodes and edges from the rule linkedlist
	      else if(line is middle condition of a rule)
 	           then 
		           add the condition to rule linkedlist
		       ...
		       then 
		           add the rule to rule linkedlist
	  		       get nodes and edges from the rule linkedlist
	      else if(line contains the last condition and consequent of rule)
		       then 
		           add the last condition and consequent to rule linkedlist
	      else if(line contains a middle condition of a rule)
		       then add the middle condition to rule linkedlist
	      else //line is empty, nothing is done
		       {}
	   ...
	   { 
	     try
	     {
		  ...
		  else //this line is not a rule case, so rule_elements_and_next_line_and_rule_case contains only one element: the next line; get the next line
		  {
	   	  ...
	   	  break;//exit loop if line is "decision tree" i.e. the end of the previous decision tree
...
static void create_graphviz_file(FileWriter out) throws IOException
  {
	 ...
	 out.write(" subgraph cluster"+Integer.toString(cluster_id)+"{\n");
	 ...
	 */
	 ...
	 for(int i=1;i<rule_size;i++)
	 {
	     ...
	     if(m1.matches())
	     {
	       ...
	       variable=m1.group(2);
	       previous_conditions_of_current_node = previous_conditions_of_current_node+","+previous_condition_of_current_node;//current node is at ith position in the linkedlist
	     ...
	     else //the item is the consequent of the rule, it is a leaf node
	     {
  	        node_id++;
	        key = previous_conditions_of_current_node;
