...
//			throw new FedXRuntimeException("Config not initialized. Call Config.initialize() first.");
	...
	private void init(String configFile) throws FedXException {
		if (configFile==null) {
			logger.warn("No configuration file specified. Using default config initialization.");
			return;
		}
		logger.info("FedX Configuration initialized from file '" + configFile + "'.");
		try {
			FileInputStream in = new FileInputStream(configFile);
			props.load( in );
			in.close();
		} catch (IOException e) {
			logger.log(Level.ERROR, "Failed to initialize FedX configuration with " + configFile + ": ", e);
	...
	public int getEnforceMaxQueryTime() {
		return Integer.parseInt( props.getProperty("enforceMaxQueryTime", "180"));
	}
	 ...
	 * The {@link QueryLog} facility allows to logger all queries to a file. See 
