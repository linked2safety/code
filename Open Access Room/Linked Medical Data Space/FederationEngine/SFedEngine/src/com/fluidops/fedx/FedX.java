...
public class FedX implements Sail {
	...
	protected static Logger logger = Logger.getLogger(FedX.class);
	...
	public void initialize() throws SailException {
		logger.debug("Initializing federation....");
		for (Endpoint member : members) {
			try {
				member.initialize();
			} catch (RepositoryException e) {
				logger.error("Initialization of endpoint " + member.getId() + " failed: " + e.getMessage(), e);
	...
	protected void shutDownInternal() throws FedXException {
		...
		for (Endpoint member : members) {
			try {
				...
				logger.error( ExceptionUtil.getExceptionString("Error shutting down endpoint " + member.getId(), e) );
				errors.add(e);
			}
		}
