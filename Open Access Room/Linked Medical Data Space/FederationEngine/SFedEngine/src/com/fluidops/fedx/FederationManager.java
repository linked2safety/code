...
import com.fluidops.fedx.evaluation.RepositoryFederationEvalStrategy;
...
public class FederationManager {
	...
	/**
	 * 
	 * @return ControlledWorkerScheduler<BindingSet>
	 */
	...
	public String getFederationSize() {
		...
		for (Endpoint e : getFederation().getMembers())
			try	{
				...
				log.error("an exception was thrown", e1);
				isLowerBound = true;
			}
	 ...
	 * 
	 * <ul>
	 *  <li>local federation: {@link RepositoryFederationEvalStrategy}</li>
	...
	protected void updateStrategy() {
		...
		if (remoteCount==0) {
			if (type!=FederationType.LOCAL) {
				...
				strategy = new RepositoryFederationEvalStrategy();		// TODO singleton?
			}
		} else if (localCount==0) {
			if (type!=FederationType.REMOTE) {
				type = FederationType.REMOTE;
				strategy = new SparqlFederationEvalStrategy();
			}
		} else {
