 ...
 * @see StatementSource
 */
public class StatementSourcePattern extends FedXStatementPattern implements StatementTupleExpr, FilterTuple {
	...
	public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(BindingSet bindings) throws QueryEvaluationException {
		...
		try {
				...
				if (t.usePreparedQuery()) {
					...
					if (preparedQuery==null) {
						try {
							...
							log.error("Error loading prefix properties: ", e1);
			...
			// XXX check if we can omit this distinct iteration
			return new DistinctIteration<BindingSet, QueryEvaluationException>(union);
