...
public abstract class FederationEvalStrategy extends EvaluationStrategyImpl {
	...
	protected static Logger log = Logger.getLogger(FederationEvalStrategy.class);
...
//			String queryNG = L2SUtil.addNamedGraphToQuery(query.getQueryString(), query.getQueryInfo().getQueryNG());
			String queryNG = L2SUtil.addNamedGraphToQuery(query.getQueryString(), QueryInfo.getNamedGraphs());
//			JOptionPane.showMessageDialog(null, "fedeval: "+queryNG);
			return source.getTripleSource().getStatements(queryNG, source.getConn(), query.getQueryInfo().getQueryType());
	...
	public Value evaluate(ConjunctiveFilterExpr node, BindingSet bindings) throws ValueExprEvaluationException, QueryEvaluationException {
		...
		for (FilterExpr expr : node.getExpressions()) {
			...
			try {
				...
				log.error("Error loading prefix properties: ", e);
				error = e;
			}
		}
