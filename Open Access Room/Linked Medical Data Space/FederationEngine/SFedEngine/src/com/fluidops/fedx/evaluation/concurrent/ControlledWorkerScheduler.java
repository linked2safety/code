...
public class ControlledWorkerScheduler<T> implements Scheduler<T> {
	...
	public void abort(ParallelExecutor<T> control) {
		...
		synchronized (taskQueue) {
			...
			int idlecount=0;
			ArrayList<WorkerThread> newWorkers = new ArrayList<WorkerThread>(workers.size());
			for (WorkerThread t : workers) {
			...
			while (!isAborted()) {
				...
				if (task!=null) {
					...
					try {
						...
						log.warn("Exception encountered while evaluating task (" + e.getClass().getSimpleName() + "): " + e.getMessage(), e);
						taskControl.toss(e);
					}
    			...
    			synchronized (taskQueue) {
    				try {
    					...
    					if (aborted) {
    						log.trace("Workerthread has been aborted.", e);
    						return;
    					}
					}
     			}
    	...
    	public void run() {
    		...
    		while (!Thread.interrupted() && !closed) {
    			...
    			synchronized (taskQueue) {
    				...
    				System.out.println("Worker Status (" + name + "): " + idle + " idle, requests in queue: " + req);
