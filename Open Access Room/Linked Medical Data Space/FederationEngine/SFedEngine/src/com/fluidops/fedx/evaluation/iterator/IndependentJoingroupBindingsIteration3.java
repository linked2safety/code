...
public class IndependentJoingroupBindingsIteration3 extends LookAheadIteration<BindingSet, QueryEvaluationException>{
	...
	protected ArrayList<BindingSet> computeResult() throws QueryEvaluationException {
		...
		ArrayList<LinkedList<BindingInfo>> aRes = new ArrayList<LinkedList<BindingInfo>>(bindings.size());
		ArrayList<LinkedList<BindingInfo>> bRes = new ArrayList<LinkedList<BindingInfo>>(bindings.size());
		...
		for (int i=0; i<bindings.size(); i++) {
			aRes.add(new LinkedList<BindingInfo>());
			bRes.add(new LinkedList<BindingInfo>());
		}
		...
		while (iter.hasNext()) {
			...
			if (bIndex==0) {
				aRes.get(bInfo.bindingsIdx).add(bInfo);
			} else if (bIndex==1) {
				bRes.get(bInfo.bindingsIdx).add(bInfo);
			} else {
		...
		for (int aidx=0; aidx<aRes.size(); aidx++) {
			LinkedList<BindingInfo> alist = aRes.get(aidx);			
			for (BindingInfo a : alist) {
				for (BindingInfo b : bRes.get(aidx)) {
