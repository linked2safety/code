...
public class FilterOptimizer extends QueryModelVisitorBase<OptimizationException> implements FedXOptimizer {
	...
	protected static Logger log = Logger.getLogger(FilterOptimizer.class);
	...
	public void meet(Filter filter)  {
		...
		if (remainingExpr.size()==0) {
			...
			// just apply the remaining condition
			filter.setCondition(remainingExpr.get(0));
		} else {
	...
	protected class FilterExprInsertVisitor extends QueryModelVisitorBase<OptimizationException> {
		...
		// if true, the current filter can be removed
		protected boolean canRemove = false;
		// the current filter Expr
		...
		private void handleFilter(FilterTuple filterTuple, FilterExpr expr) {
			...
			// if: filter contains all variables => push filter
			// else: filter is still needed for post filtering
