...
public class RemoteRepositoryGraphRepositoryInformation extends RepositoryInformation {
	 ...
	 * @param repNode
	 */
	public RemoteRepositoryGraphRepositoryInformation(Graph graph, Resource repNode) {
	...
	protected void initialize(Graph graph, Resource repNode) {
		...
		Iterator<Statement> iter = graph.match(repNode, new URIImpl("http://fluidops.org/config#repositoryServer"), null);
		...
		// repositoryName
		iter = graph.match(repNode, new URIImpl("http://fluidops.org/config#repositoryName"), null);
		String repoName = iter.next().getObject().stringValue();
